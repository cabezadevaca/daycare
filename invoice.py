...
def save_pdf(invoice, daycare, directory=None):
    ...
    for child in parent.children:
        ...
        pdf.multi_cell(50, 10, str(",".join(Invoice._make_date_range(child.current_attendance_dates))))#, 1,0, 'C')
        # pdf.set_font('Arial', '', 12)
    ...
    pdf_output_filename = f'ICleverInvoice_{name}_{invoice.month}_{invoice.year}.pdf'
...
def save_family_pdf(invoice, daycare, directory=None):
    ...
    family = invoice.parent

    # Title
    pdf.ln(30)  # Add a line break


    parents = [str(x) for x in family.parents]

    pdf.chapter_title(f'Parent: {", ".join(parents)}')
    pdf.cell(0, 5, f'Invoice Period: {invoice.month}/{invoice.year}', 0, 1)
    pdf.cell(0, 5, f'Generate on: {invoice.gen_date}', 0, 1)
    ...
    for child in family.children:
        dates = Invoice._make_date_range(child.current_attendance_dates)
        pdf.cell(30, 10, str(child.name), 1,0, 'C')
        pdf.set_font('Arial', '', 8)
        pdf.cell(50, 10, str(",".join(dates)), 1,0, 'C')
        # pdf.multi_cell(50, 10, str(",".join(dates)), 1)#, 0, 'C')
        pdf.set_font('Arial', '', 12)
        pdf.cell(30, 10, str(len(child.current_attendance_dates)), 1,0, 'C')
        # pdf.multi_cell(30, 10, str(len(child.current_attendance_dates)), 1)#, 0, 'C')
        pdf.cell(30, 10, '${:,.2f}'.format(child.day_rate), 1,0, 'C')
        pdf.cell(30, 10, '${:,.2f}'.format(child.current_fee), 1,0, 'C')
        pdf.ln()
    ...
    name = family.parents[0].name.replace(' ', '_')



    pdf_output_filename = f'{daycare.invoice_prefix}Invoice_{name}_{invoice.month}_{invoice.year}.pdf'
    if directory is not  None:
        pdf_output_filename = os.path.join(directory, pdf_output_filename)
    pdf.output(pdf_output_filename)
    return pdf_output_filename
...
class Invoice(object):
    ...
    @staticmethod
    def generate_invoice(year, month, parent, public_holidays, first_day=None, last_day=None):
        days_count = 0
        fee = 0
        for child in parent.children:
            days = get_attendance_count(year, month, child.child_schedule, public_holidays, first_day, last_day)
    ...
    @staticmethod
    def generate_family_invoice(year, month, fam, public_holidays, first_day=None, last_day=None):
        days_count = 0
        fee = 0
        for child in fam.children:
            ...
            fee += child.current_fee
            days_count += len(days)
        invoice = Invoice(year, month, fam, public_holidays)
        invoice.fee = fee
        invoice.days_count = days_count
        return invoice
